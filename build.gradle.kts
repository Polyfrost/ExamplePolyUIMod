...
dependencies {
    for (dep in listOf("nanovg", "tinyfd")) {
        val lwjglDep = "org.lwjgl:lwjgl-$dep:3.3.1"
        implementation(lwjglDep) {
            isTransitive = false
        }
        for (native in listOf("windows", "windows-arm64", "linux", "macos", "macos-arm64")) {
            implementation("$lwjglDep:natives-$native") {
                isTransitive = false
            }
        }
    }
    implementation("org.polyfrost:polyui:1.0.2")
    ...
    setOf(
        ...
        modImplementation(fabricApi.module(it, "0.91.0+${platform.mcVersionStr}"))
    }
